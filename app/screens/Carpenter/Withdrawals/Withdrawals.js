...
export default Withdrawals = ({ navigation }) => {
    ...
    const pageApiCall = async () => {
        ...
        setLoadingData(true)
        const data = await Config.ServerCalls.PostApiCall(Config.ServerCallUrls.GET_WITHDRAW_LIST, JSON.stringify(payload), {})
        setLoadingData(false)
        if (data.data.withdrawrequest.length > 0) {
            setListData(data.data.withdrawrequest)
            setNoData(false)
            setNoMoreData(false)
            setPaging(2)
        } else {
    ...
    const getListNextPage = async () => {
        ...
        const data = await Config.ServerCalls.PostApiCall(Config.ServerCallUrls.GET_WITHDRAW_LIST, JSON.stringify(payload), {}, false)
        setMoreDataLoader(false)
        if (data && data.data.withdrawrequest.length > 0) {
            setListData([...listData, ...data.data.withdrawrequest])
            setPaging(paging + 1)
        } else {
            setNoMoreData(true)
        }
        ...
        return (
            <View style={Styles.listItemView}>
                <View style={{ flex: 1 }}>
                    ...
                    <Component.CustomText style={{ color: getColorPaymentStatus(item.status) }}>Status: {getStringPaymentStatus(item.status)}</Component.CustomText>
